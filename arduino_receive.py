#!/usr/bin/env python3
import serial
import serial.threaded
import struct
import time

port = "/dev/ttyACM0"
baudrate = 115200

class SDReader(serial.threaded.Protocol):
    crc8_table = [
        ]
    
    @staticmethod
    def crc8_calc(data):
        crc = 0xFF
        for b in data:
            crc = SDReader.crc8_table[crc ^ b]
        return crc

    def __init__(self):
        self.buffer = bytes()

    def data_received(self, data):
        self.buffer += data
        while True:
            index = self.buffer.find(b'\xa5')
            if index == -1:
                self.buffer = bytes()
                break
            self.buffer = self.buffer[index:]
            if len(self.buffer) < 3:
                break
            size = self.buffer[1]
            if size > 252:
                self.buffer = self.buffer[1:]
                continue
            if len(self.buffer) < size + 3:
                break
            crc = self.buffer[2]
            if crc != SDReader.crc8_calc(self.buffer[3:3+size]):
                self.buffer = self.buffer[1:]
                continue
            self.handle_packet(self.buffer[3:3+size])
            self.buffer = self.buffer[3+size:]

    def handle_packet(self, packet):
        print("Packet received: ", repr(packet))

class DataPrinter(SDReader):
    def __init__(self):
        self.skip = 100
        self.prev = 0
        self.disconnects = -1
        super().__init__()
    
    def handle_packet(self, packet):
        if len(packet) != 6:
            return
        if self.skip > 0:
            self.skip -= 1
            return
        s, m, a = struct.unpack('HHH', packet)
        if s != (self.prev + 1) % 65536:
            self.disconnects += 1
        self.prev = s
        print(f"{s}\t{m}\t{a}\t{self.disconnects}")

ser = serial.Serial(port, baudrate, timeout=5)

with serial.threaded.ReaderThread(ser, DataPrinter) as protocol:
    time.sleep(90)

print("Done")
